...
def nantes_toulouse_consolidate_station_data() -> None:
    ...
    for city, city_code in zip(cities, cities_code):
        ...
        raw_data_df["city_code"] = get_city_code(
            city
        ...
        con.execute(
            "INSERT OR REPLACE INTO CONSOLIDATE_STATION SELECT * FROM station_data_df;"
        )
        con.close()
...
def strasbourg_consolidate_station_data() -> None:
    ...
    strasbourg_raw_data_df["id_CONSOLIDATE_STATION"] = strasbourg_raw_data_df[
        "id"
    ].apply(lambda x: f"{STRASBOURG_CITY_CODE}-{x}")
    ...
    con.execute(
        "INSERT OR REPLACE INTO CONSOLIDATE_STATION SELECT * FROM strasbourg_station_data_df;"
    )
