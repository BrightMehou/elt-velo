...
import requests
from minio_utils import store_json
    ...
    for city, url in urls.items():
        try:
            ...
            if response.status_code == 200 and response.text.strip():
                ...
                logger.warning(f"⚠️ {city} indisponible (status: {response.status_code}), création fichier vide.")
...
def get_commune_data() -> None:
    ...
    try:
        ...
        if response.status_code == 200 and response.text.strip():
            ...
            logger.warning(f"⚠️ API communes indisponible (status: {response.status_code}), création fichier vide.")
